...
// Constants
    ...
    private array $elementTypeConfig = [
        ...
        'craft\elements\ContentBlock' => [
            'get' => 'getContentBlockElements',
            'sort' => '01',
        ],
        'craft\elements\GlobalSet' => [
            'get' => 'getGlobalSetElements',
            'sort' => '99',
        ],
        'craft\elements\Category' => [
            'get' => 'getCategoryElements',
            'sort' => '10',
        ],
        // 'craft\elements\Tag' => [
        //     'get' => 'getTagElements',
        ...
        'craft\elements\Address' => [
            'get' => 'getAddressElements',
            'sort' => '21',
        ],
        'craft\commerce\elements\Product' => [
            'get' => 'getProductElements',
            'sort' => '30',
        ],
        'craft\commerce\elements\Variant' => [
            'get' => 'getVariantElements',
            'sort' => '35',
        ],
        'putyourlightson\campaign\elements\CampaignElement' => [
            'get' => 'getCampaignElements',
            'sort' => '40',
        ],
        'benf\neo\elements\Block' => [
            'get' => 'getNeoBlockElements',
            'sort' => '02',
        ],
    ];
    ...
    public function getIncomingElements(Element $element, int $siteId)
    {
        ...
        if ($this->hasEventHandlers(static::EVENT_ADD_ELEMENTS)) {
            ...
            $this->trigger(static::EVENT_ADD_ELEMENTS, $event);
            if ($event->data) {
    ...
    private function getAddressElements($elementIds, $siteId)
    {
        ...
        foreach ($elements as $element) {
            ...
            $results[] = [
                'id' => $element->id,
                'icon' => '@appicons/map-location.svg',
                'color' => 'var(--black)',
                'title' => $title,
                'url' => $rootOwner->cpEditUrl,
                'sort' => $this->elementTypeConfig[get_class($element)]['sort'],
    ...
    private function getExtraText($element, $type)
    {
        ...
        if ($element->isProvisionalDraft) {
            $parts[] = Craft::t('_elementmap', 'Provisional Draft');
            $user = User::findOne($element->creatorId);
            if ($user) {
                $parts[] = $user->username;
            }
        } elseif ($element->getIsDraft()) {
            $parts[] = Craft::t('_elementmap', 'Draft');
            $parts[] = $element->draftName;
        } elseif ($element->getIsRevision()) {
            $parts[] = Craft::t('_elementmap', 'Revision') . ' ' . $element->revisionNum;
        }
    ...
    private function getNestedElementText(ElementInterface $element)
    {
        ...
        try {
            ...
            // May not work for all nested element types
            $text = '';
        }
        return $text;
    }

    protected function getTitleForElement(ElementInterface $element, ?ElementInterface $rootOwner = null): string
    {
        $title = $element->title;
        ...
        if ($rootOwner->id != $element->id) {
            $nestedElementText = $this->getNestedElementText($element);
            $title = ($rootOwner->title ?? $rootOwner->name ?? $rootOwner->id) .
                ($nestedElementText ? (' -> ' . $nestedElementText) : '');
        }
